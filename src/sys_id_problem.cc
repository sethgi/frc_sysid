...
void SysIdProblem::processLog(const std::string &base_path)
{
  ...
  auto apriltag_data = load_csv(base_path + "/fiducials.csv");           // Header: time, target_x, target_y, target_z, target_roll, target_pitch, target_yaw
  ...
  for (const auto &row : apriltag_data) {
    ...
    const int tag_id = row[1];

    Eigen::VectorXd tag_vec(7);
    for (int i = 0; i < 7; i++) {
      tag_vec(i) = row[i+2];
    }
    // The Python code passed "None" for tag id; here we use a dummy id (0).
    april_tag_manager.registerMeasurement(ts, tag_id, tag_vec);
  }
}
