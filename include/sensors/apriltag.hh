...
public:
  ...
  struct Measurement {
    double time;
    int tag_id;
    Eigen::Vector3d bearing_vec;
    double range;
  };

  AprilTagManager(const RobotConfig &robot_config, const FieldConfig &field_config);

  void registerMeasurement(double time, int tag_id, const Eigen::VectorXd &meas_vec);
  ...
  std::vector<Measurement> getMeasurementsCameraFrame() const;
