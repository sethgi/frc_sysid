...
from logreader import DataLogReader
...
def dump_data_to_csv(log_file, out_dir, time_limit: float = None):
    IMU, encoders, tags, botposes, fiducials = load_data(log_file, time_limit)
    ...
    with open(f"{out_dir}/fiducials.csv", "w", newline="") as f:
        writer = csv.writer(f)
        header=["time", "tag_id", "azimuth", "elevation", "area", "distToCamera", "distToRobot", "ambiguity"]
        writer.writerow(header)
        for t, data in fiducials:
            if np.sum(np.abs(data)) > 1e-3:
                writer.writerow([t] + list(data[:len(header)-1]))
